...
#now run on z_score (unweighted value)
ap <- ap %>% arrange(desc(z_tot))
ap1 <- ap

ap1 <- head(ap1, -7)  #remove last 6 ROWS (that's where it tails off)
ap1$z_rank <- rank(ap1$z_tot)  #rank var
#ggplot(ap1, aes(rank(z_tot), z_tot)) + geom_point()

#z_tot shift var
ap1$z_shift <- ap1$z_tot - min(ap1$z_tot)

#now try 15
ap1$z_shift3 <- ap1$z_shift + 15

I <- 1.1405
ap1$raw_price <- I ^ ap1$z_shift3 - 4
sum(ap1$raw_price)
ggplot(ap1, aes(z_tot, raw_price)) + geom_point()

ap1_last <- ap[104:110,]
ap1_last$raw_price <- 3

ap1 <- ap1 %>%
  full_join(ap1_last) %>%
  arrange(desc(z_pos))

#remove unnecessary df's
rm(ap_last, ap1_last, relievers, relievers1, relievers2, starters, starters1, starters2)
rm(stat1, stat2, stat3, stat4, stat5, df)
rm(ap)

#read in numbers for dynasty prices
...
#keep "regulars", those players who are going to start more days than not
hitters_reg <- hitters %>%
  filter(pa >= 250) %>%  #will not want players with less than half a season of at bats, so filter for this
  arrange(name)
...
#remove uneeded df's
rm(stat1, stat2, stat3, stat4, stat5, stat6, df)
...
#one min value guy per team
ap_last <- ap[353:368,]
ap_last$dyn_price <- 3

#exponential for the rest
ap_rest <- ap[1:352,]
#shift the values
ap_rest$shift <- ap_rest$z_pos - min(ap_rest$z_pos)

ggplot(ap_rest, aes(rank(z_pos), z_pos)) + geom_point()

ap_rest$shift1 <- ap_rest$shift + 40

A <- 1.0748532
ap_rest$dyn_price <- A ^ ap_rest$shift1 - 15
sum(ap_rest$dyn_price)
ggplot(ap_rest, aes(z_pos, dyn_price)) + geom_point()

#join with end of df
ap_rest <- ap_rest %>%
  full_join(ap_last)
#select cols for joining
ap <- ap_rest %>%
  select(name, dyn_price)

#bind with available players
ap <- ap1 %>%
  left_join(ap, by = "name")

#select rows I want
df <- ap %>%
  select(name, team, pos, raw_price, z_tot, dyn_price, z_pos, auction_price)
df$auction_price <- round(df$auction_price, 2)
df$raw_price <- round(df$raw_price, 2)
df$dyn_price <- round(df$dyn_price, 2)

#filter by positions
df_sp <- df %>% filter(pos == "sp")
df_rp <- df %>% filter(pos == "rp")
df_c <- df %>% filter(pos == "2")
df_1b <- df %>% filter(pos == "3")
df_2b <- df %>% filter(pos == "4")
df_3b <- df %>% filter(pos == "5")
df_ss <- df %>% filter(pos == "6")
df_of <- df %>% filter(pos == "7")
df_mi <- df %>% filter(pos == "4" | pos == "6")
df_ci <- df %>% filter(pos == "3" | pos == "5")
df_hitters <- df %>% filter(pos == "2" | pos == "3" | pos == "4" | pos == "5" | pos == "6" | pos == "7")
#export file
write.csv(df, "C:/Users/Ben/Desktop/R projects/2018_my_draft_prices.csv")
write.csv(df_sp, "C:/Users/Ben/Desktop/R projects/2018_my_draft_prices_sp.csv")
